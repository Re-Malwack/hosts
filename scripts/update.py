...
# Source URLs to split OUT of their hosting profile into a dedicated "<profile>-malware"
# file, so no single profile file exceeds GitHub's 100 MB file size limit.
MALWARE_SOURCE_URLS = {
...
def main():
    ...
    PROFILE_DESCRIPTIONS = {
        ...
        "aggressive-malware": "Extended malware domain list, split out of the Aggressive profile to stay within GitHub's file size limit. Combine with Aggressive for full coverage."
    }
    ...
    def build_profile(profile, sources):
        ...
        header_lines = [
            f"# Re-Malwack Hosts - {profile.replace('-', ' ').title()} Profile",
        ...
        return f"| **{profile.replace('-', ' ').title()}** | {desc} | `{total_count:,}` | {sources_html} | `{raw_link}` |"

    for profile, sources in profile_sources.items():
        print(f"\nBuilding profile: {profile}")
        main_sources = [s for s in sources if s["url"] not in MALWARE_SOURCE_URLS]
        malware_sources = [s for s in sources if s["url"] in MALWARE_SOURCE_URLS]

        row = build_profile(profile, main_sources)
        if row: profile_table_rows.append(row)

        for ms in malware_sources:
            split_profile = f"{profile}-malware"
            print(f"\nBuilding profile: {split_profile}")
            row = build_profile(split_profile, [ms])
            if row: profile_table_rows.append(row)
