...
def build_blocklist_file(name, sources, is_whitelisted, output_dir):
    ...
    return f"| **{display_name}** | `{total_count:,}` | {sources_html} | `{raw_link}` |", stat
...
def main():
    ...
    blocklist_names = ["porn", "gambling", "fakenews", "social", "malware"]
    blocklist_sources = {}
    ...
    for bl_name in blocklist_names:
        ...
        row, stat = build_blocklist_file(bl_name, blocklist_sources[bl_name], is_whitelisted, output_dir)
        if row: blocklist_table_rows.append(row)
        blocklists_stats.append(stat)

    # Build trackers: base (general trackers only)
    print(f"\nBuilding blocklist: trackers")
    row, stat = build_blocklist_file("trackers", trackers_base, is_whitelisted, output_dir)
    if row: blocklist_table_rows.append(row)
    blocklists_stats.append(stat)
    ...
    for brand, brand_src in trackers_brands.items():
        ...
        print(f"\nBuilding blocklist: {bl_name}")
        row, stat = build_blocklist_file(bl_name, combined_sources, is_whitelisted, output_dir)
        if row: blocklist_table_rows.append(row)
        blocklists_stats.append(stat)

    update_stats_json("blocklists", blocklists_stats)
